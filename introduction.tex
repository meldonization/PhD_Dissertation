...
个周期以上便被认为是真实的天体信号。}。不过，恒星光谱学依然在其间飞速发展，如 HD 114762 
...
\cite{WolszczanFrail1992}。而此时 Walker 等人则在对主序恒星长达 12 年的视向速度监测中，
...
观测系外行星相较恒星或太阳系内行星，通常需要用到非常高分辨率、灵敏度和稳定性的仪器，
也因而会遇到诸多困难，甚至还得排除恒星自身活动的干扰。比如测量围绕类太阳的系外地球
需要精度为 1 m$\odot$s$^{-1}$的视向速度测量，相当于使用分辨率为 R = 100,000 的光谱仪器探测
...
在简单的二体运动中（\S \ref{apdx:twobodyproblem}），可以得到一颗轨道半长径为 $a$，质量
为 $M_\tif{p}$ 的行星，可对质量为 $M_\tif{s}$ 的宿主恒星造成如下大小的半振幅视向速度 $K_1$
（单位：米每秒）：
...
如凌星计时变化（Transit Timing Variation，简称 TTV）或者 RV。}。目前地面上比较成熟的巡天项
...
双行星系统\footnote{数据截至 2017 年 2 月，参见 NASA Exoplanet Archive：\url{http://exoplanetarchive.ipac.caltech.edu/}}。
...
例\cite{Haisch2001}，发现原行星盘的存活时标在不到百万年（约 10 Myr）的量级（见图 \ref{fig:pftimescale}）。
这对类木行星行形有着非常重要的限制，因为如今普遍认为木星必须在气体盘消散前吸收足够的气体
以长成如今的质量大小\cite{WilliamsCieza2011}。关于星周盘的详细介绍请参考 \S \ref{chapter:form_evo}。

\begin{figure}[t]
\centering
\includegraphics[width=1.0\textwidth]{figures/chapter1/fig10_discevo.eps}
\caption[原行星盘演化中由 (a) 至 (d) 四个主要过程，分别为活跃吸积阶段、固体沉降阶段、盘消散阶段与残骸盘阶段。图片版权 Williams 和 Cieza。]{原行星盘演化中由 (a) 至 (d) 四个主要过程，分别为活跃吸积阶段、固体沉降阶段、盘消散阶段与残骸盘阶段。图片取自文献\citen{WilliamsCieza2011}。 }
\label{fig:discevo}
\end{figure}
...
理参数，由此表可粗略看出太阳系各大行星具有非常好的一致共面性，除了水星以外都处于近圆轨道，
且在火星和木星之间存在明显的物态性质差异。如果将每个行星的重元素平均到打散至行星之间的
...
认为原行星核通过吸积气体（core accretion）而形成\cite{Mizuno1980,BodenheimerPollack1986,Pollack1996}，
...
（椭圆实线），冷木星族（方框实线）和超级地球族（椭圆虚线）。

\begin{figure}[t]
\centering
\includegraphics[width=1.0\textwidth]{figures/chapter1/fig13_exompplot.pdf}
\caption{现今探测到系外行星的周期 --- 质量散点图。其中不同颜色分别代表不同的探测方法：青色代表视向速度法，灰色是凌星法，黄色指代微引力透镜法，红色代表直接成像法，紫色则表示其他方法。黑色字母 Me，V，E，Ma，J，S，U，N 则分别表示太阳系内水星，金星，地球，火星，木星，土星，天王星和海王星。本文将系外行星进行人为类别划分包络在封闭曲线内，分别为热木星族（椭圆实线），冷木星族（方框实线）和超级地球族（椭圆虚线）。}
...
\subsubsection{冷木星族  Cold/Normal Jupiter Population}

与轨道距离主星较近的热木星相对应有一类行星被称作冷木星（或者常规类木星）。
...
\cite{Kuiper1951,Cameron1978,Boss1997}则可比较合理地解释这些巨行星是如何形成于
...
超级地球，有时又称迷你海王星（mini-Neptune），对于 RV 探测到的系外行星一般定义为
介于十个地球质量与海王星质量之间，或者对于凌星法探测到的行星，则其半径大约介于
...
那么超级地球的形成历史究竟为何般？它们又是否可归至类地行星范畴呢？前面 \S 
...
到如今也尚未弄清其中的动力学机制，以及此现象是否依赖于超级地球的形成过程。
...
\cite{Pont2009}表明它的起源很可能与 Lidov-Kozai 机制\cite{Lidov1962,Kozai1962}有着密切关联
...
长周期冷木行星\cite{NeveuVanMalle2016}。这和传统的认为热木星更倾向于成单的想法截然不同
...
此外，本文将于 \S \ref{chapter:data_stat} 章详细探讨系外热木星系统的统计性质，回顾解释这类行
星形成的困难之处。最后还包括如何利用轨道-自转不共面性（spin-orbit misalignment ）来限制这些
系统的潮汐演化过程和参数，以及这些过程和参数蕴含或还原了哪些系统演化的物理效应。在本文的
最后一部分，将会总结与展望系外行星领域的未来计划。而如何从宏观层面研究行星系统的形成与演
化以及从微观层面去细致描述行星的性质，这些研究将有利于击破现有的系外行星难关。
